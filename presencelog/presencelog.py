...
class PresenceLog(commands.Cog):
    ...
    async def on_presence_update(self, before: discord.Member, after: discord.Member):
        ...
        if before.status != after.status:
            updates.append(
                f"🟢 **{after.display_name}({after.mention})** changed status: `{before.status.name}` → `{after.status.name}`"
            )
        ...
        if before_act != after_act:
            updates.append(
                f"🎮 **{after.display_name}({after.mention})** changed activity: `{before_act or '-'}` → `{after_act or '-'}`"
            )
